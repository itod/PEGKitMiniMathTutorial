...
- (id)init {
    ...
    if (self) {
        ...
        self.tokenKindTab[@")"] = @(MINIMATH_TOKEN_KIND_CLOSE_PAREN);

        self.tokenKindNameTab[MINIMATH_TOKEN_KIND_STAR] = @"*";
        self.tokenKindNameTab[MINIMATH_TOKEN_KIND_OPEN_PAREN] = @"(";
        self.tokenKindNameTab[MINIMATH_TOKEN_KIND_PLUS] = @"+";
        self.tokenKindNameTab[MINIMATH_TOKEN_KIND_CLOSE_PAREN] = @")";
...
- (void)addExpr_ {
    ...
    [self multExpr_]; 
    while ([self speculate:^{ [self match:MINIMATH_TOKEN_KIND_PLUS discard:YES]; [self multExpr_]; }]) {
...
- (void)primary_ {
    ...
    if ([self predicts:TOKEN_KIND_BUILTIN_NUMBER, 0]) {
        [self atom_]; 
    } else if ([self predicts:MINIMATH_TOKEN_KIND_OPEN_PAREN, 0]) {
        [self match:MINIMATH_TOKEN_KIND_OPEN_PAREN discard:YES]; 
        [self expr_]; 
        [self match:MINIMATH_TOKEN_KIND_CLOSE_PAREN discard:YES]; 
    } else {
